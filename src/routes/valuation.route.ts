...
const LOCATION_SERVICE_URL = process.env.LOCATION_SERVICE_URL || "localhost:3002";
const BIKE_PRICE = 8;
const CAR_PRICE = 14;
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    const response = await axios.post(LOCATION_SERVICE_URL, {fromAddress,toAddress});
    if(!response){
    ...
    res.status(200).send({cost, rider_commission, tax})
  }
);
